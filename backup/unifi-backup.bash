...
REMOTE_BACKUP_DIR_DEVICE="$REMOTE_BACKUP_DIR/$BALENA_DEVICE_NAME_AT_INIT"
...
do
  if [[ "$(find $LOCAL_BACKUP_DIR -type f)" ]]; then
    ...
    if [[ ${BACKUP_HASH:-} == "$(find $LOCAL_BACKUP_DIR -type f | shasum)" ]]; then
      continue
    fi
    ...
    # Save the state
    BACKUP_HASH=$(find $LOCAL_BACKUP_DIR -type f | shasum)
  else
